...
class MinaraitaiCharacter extends Character {
  // (コンストラクタ：キャラクターの初期化を行う)
  MinaraitaiCharacter(float x, float y, CharacterCountManager countManager) {
    // (スーパークラス（Character）のコンストラクタを呼び出す。)
    //キャラクター名、位置、属性を設定する
    
    /*☆ここは設定してください。⭐☆*/
    super("Minaraitai Character", x, y, 2, countManager); // 属性0はFire, 属性1はWater, 属性2はGrass
    /*☆                   ⭐☆*/
  }
  ...
  void initializeStats() {
    /*☆ここは設定してください。⭐☆*/
    //全部で合計200までしか値は振れません。
    this.hp = 1;           // HPを100に設定 (HPは1〜200まで)
    this.attackPower = 100;   // 攻撃力を50に設定 (攻撃力は1〜100まで)
    this.speed = 99;         // 速度を50に設定 (速度は1〜100まで)
    /*                    ⭐☆*/
